...
"
" ---------------------------------- "
" dictionary for mapping line comment tokens to the corresponding files
let g:line_comment_dict = {
		\'//': ["js", "ts", "cpp", "c", "dart"],
		\'#': ['py', 'sh'],
		\'"': ['vim', ''],
		\}
...
" ---------------------------------- "
"   function for line auto commenting
function! s:AutoLineComment()
"   get extension    
    let extension = expand('%:e')
    let comment = g:default_line_comment 

"   check file extension against each entry in line comment dictionary    
    for item in items(g:line_comment_dict) 
...
" ---------------------------------- "
"  define commands
command! -range AutoLineComment <line1>,<line2>call <sid>AutoLineComment()
...
if (g:autocomment_map_keys)
    vnoremap <silent><C-_> :AutoLineComment<CR>
    nnoremap <silent><C-_> :AutoLineComment<CR>
